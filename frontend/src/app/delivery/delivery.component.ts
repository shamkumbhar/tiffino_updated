...
export class DeliveryPartnerComponent implements OnInit {
  ...
  getAllPartners() {
    this.http.get<any[]>(`${this.apiUrl}/delivery-partners/getall`).subscribe({
      next: (res) => {
        console.log(res);
        this.data = res;
  ...
  postdeliverypartners() {
    ...
    this.http.post(`${this.apiUrl}/delivery-partners/create`, payload).subscribe({
      next: (res) => {
  ...
  getactiveplan() {
    this.subs.getActivePlans().subscribe({
      next: (data) => {
        console.log(data);
        const activePlan = data.find((plan: any) => plan.isActive);
        if (activePlan) {
